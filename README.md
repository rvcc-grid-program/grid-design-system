...
- `docs/icons/generated/` — the five content-icon PNGs for Canvas
  (`<use>@3x.png`, transparent, light-theme ink baked in). Generated by
  `pnpm run build:icons` from `pipeline/icons.js` and committed so the
  Pages site hosts them; Canvas output references them absolutely.
- `pipeline/` — the wiki→HTML builds: `markdown.js` (the markdown contract),
  `enhance.js` (shared DOM transforms), `icons.js` (the Lucide content-icon
  registry: inline SVG for web, PNG swap for Canvas), `build-icons.js` (the
  rasterizer), `templates.js` (page shells), `config.js` (consumer
...
```bash
pnpm add "github:rvcc-grid-program/grid-design-system#v1.2.0"
...
pnpm run canvas <page.md>    # Canvas-ready inline-styled fragment (regenerates icon PNGs first)
...
- **Published Canvas content consumes almost nothing at runtime** — every
  byte of CSS is baked in at build time. The one exception (since the
  content-icon system, 2026-07-04): pages load the five icon PNGs from this
  repo's Pages site, so `docs/icons/generated/` must stay published at
  stable URLs. Text and structure never depend on them (decorative,
  `alt=""`), so a missing PNG degrades to an empty tile, not broken
  content.
