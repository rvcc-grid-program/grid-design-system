...
- Whether **`overflow-wrap`** survives, in either direction. No paste test has
  ...
  (DECISIONS.md, 2026-07-30) to opt a prose label out of mid-word breaking.
  **Low stakes as of v1.8.1**, which dropped the pill from that link: with no
  border or tint, a label that breaks mid-word is slightly ugly rather than
  visibly broken, so nothing structural rides on the verdict. Same test should
